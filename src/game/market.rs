...
pub fn gen_building_tables() -> HashMap<String, Building> {
    ...
    all_buildings.insert(
        "Dwarven Buildings".to_string(),
        Building {
            input: quick_hash(vec![(Resources::Water, 10)]),
            output: quick_hash(vec![(Resources::Food, 20)]),
            image_sylt_id: Some("mushroom_farm".to_string()),
        },
    );
